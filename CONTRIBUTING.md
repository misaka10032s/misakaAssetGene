...
1. `.claude/context/spec.md`：產品與架構規格的單一事實來源
...
1. 新需求或重大變更，先和 `architect` 角色對照 `.claude/context/spec.md` 討論可行性，再提交實作。
2. 如果你的變更會影響功能、流程或 repo 邊界，請同步更新 `.claude/context/spec.md`、`.claude/CLAUDE.md` 或 `.claude/` 文件。
...
- 變更內容與 `.claude/context/spec.md` 一致
